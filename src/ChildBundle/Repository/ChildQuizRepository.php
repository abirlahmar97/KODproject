 ...
 */
class ChildQuizRepository extends \Doctrine\ORM\EntityRepository
{
...
//    }
    ...
    public function quizTime($child){
        $em = $this->getEntityManager();
        $conn = $em->getConnection();
        $query = $conn->prepare("SELECT SUM(Time_TO_SEC(cq.duration)) as played_time " .
            "FROM child_quiz cq WHERE child_id = :id");
        $query->bindValue('id', $child );
        $query->execute();
        return $query->fetchAll();
    }
